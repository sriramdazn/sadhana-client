...
export type CompletedDailyCounts = Record<string, Record<string, number>>;

export function migrateCompletedStorageToDailyCounts(raw: any): CompletedDailyCounts {
  ...
  // New shape: { [dateIso]: { [sadanaId]: number|boolean } }
  const out: CompletedDailyCounts = {};
  for (const dateIso of topKeys) {
    ...
    for (const sadanaId of Object.keys(v)) {
      const n = v[sadanaId];
      if (typeof n === "number") {
        if (n > 0) dayMap[sadanaId] = Math.min(2, Math.max(0, n));
